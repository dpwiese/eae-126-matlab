...
% Project 7 - Part 1 - 2-D Supersonic Flow: LINEAR MACH NUMBER
% Daniel Wiese
...
for i = nLE:nx/2
    yBT(i) = slop*x(i)+tau*chord/2;
    yBB(i) = -slop*x(i)-tau*chord/2;
end
...
for Maloop = 1:1
    ...
    for i = nLE
        ...
        for j = ny/2;
            ...
            Cu(j) = 0;
            Du(j) = -BS(i,j)*(-2*u(i-1,j)+u(i-2,j))/dx^2+uinf*sin(alpha)/(dx*dy)-(yBB(nLE+1)-yBB(nLE))/(dy*dx^2);
        end
        ...
        for j = ny/2+1;
            ...
            Bu(j) = -1/dy^2+BS(i,j)/dx^2;
            Cu(j) = 1/dy^2;
            Du(j) = -BS(i,j)*(-2*u(i-1,j)+u(i-2,j))/dx^2-uinf*sin(alpha)/(dx*dy)+(yBT(nLE+1)-yBT(nLE))/(dy*dx^2);
        end
    ...
    for i = nTE
        ...
        for j = ny/2;
            ...
            Bu(j) = -1/dy^2+BS(i,j)/dx^2;
            Cu(j) = 0;
            Du(j) = -BS(i,j)*(-2*u(i-1,j)+u(i-2,j))/dx^2-uinf*sin(alpha)/(dx*dy)+(yBB(nTE)-yBB(nTE-1))/(dy*dx^2);
        end
        ...
        for j = ny/2+1;
            ...
            Bu(j) = -1/dy^2+BS(i,j)/dx^2;
            Cu(j) = 1/dy^2;
            Du(j) = -BS(i,j)*(-2*u(i-1,j)+u(i-2,j))/dx^2+uinf*sin(alpha)/(dx*dy)-(yBT(nTE)-yBT(nTE-1))/(dy*dx^2);
        end
