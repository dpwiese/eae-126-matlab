...
% Project 8/9 - Transonic Flow - SUBSONIC
% Daniel Wiese
    ...
    for i = 3:nx-1
        ...
        while iter<itermax
            for j = 1;
                ...
                Cu(j) = 1/dy^2;
                Du(j) = (-1/dx^2)*(TKp1*u(i+1,j)+TKm1*u(i-1,j)) - (fp1-2*fm0+fm1)/dx^2 + (TKm1*u(i-1,j)-2*TKm0*u(i,j) + TKp1*u(i+1,j))/dx^2 + d2yBdx2(i)/dy;
            end
            for j = 2:ny-1
                fm1 = (1-Minf^2)*u(i-1,j)-((gamma+1)*Minf^2*u(i-1,j)^2)/2;
                fm0 = (1-Minf^2)*u(i,j)-((gamma+1)*Minf^2*u(i,j)^2)/2;
                ...
                Cu(j) = 1/dy^2;
                Du(j) = (-1/dx^2)*(TKp1*u(i+1,j)+TKm1*u(i-1,j)) - (fp1-2*fm0+fm1)/dx^2 + (TKm1*u(i-1,j)-2*TKm0*u(i,j) + TKp1*u(i+1,j))/dx^2;
            end
            for j = ny
                Au(j) = 0;
                Bu(j) = 1;
